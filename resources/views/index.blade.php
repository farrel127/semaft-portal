                            ...
                            <tbody class="text-sm divide-y divide-gray-100">
                                @forelse($aspirasis as $item)
                                    <tr class="hover:bg-blue-50/50 transition">
                                        ...
                                        <td class="p-4">
                                            <p class="font-bold text-semaft-navy">{{ $item->nama }}</p>
                                        ...
                                        <td class="p-4 text-center whitespace-nowrap">
                                            ...
                                            <form action="{{ route('aspirasi.update', $item->id) }}" method="POST">
                                                ...
                                                <select name="status" onchange="this.form.submit()" class="text-xs font-bold rounded-full px-3 py-1 border-0 ring-1 ring-inset focus:ring-2 focus:ring-inset focus:ring-semaft-navy cursor-pointer
                                                    {{ $item->status == 'Menunggu' ? 'bg-red-50 text-red-700 ring-red-600/20' : '' }}
                                                    {{ $item->status == 'Dibaca' ? 'bg-yellow-50 text-yellow-700 ring-yellow-600/20' : '' }}
                                                    {{ $item->status == 'Selesai' ? 'bg-green-50 text-green-700 ring-green-600/20' : '' }}">
                                        ...
                                        <td class="p-4 text-center whitespace-nowrap">
                                            <!-- Tombol Hapus -->
                                            <form action="{{ route('aspirasi.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pesan ini?');">
                                                @csrf
                                                @method('DELETE')
